...
function PermRow({
  ...
  const BitBox = ({ bit, value, onToggle, color }: { bit: string; value: boolean; onToggle: () => void; color: string }) => (
    <button
      onClick={onToggle}
      className={`w-10 h-10 rounded-md flex items-center justify-center font-mono text-sm font-bold transition-colors border
        ${value
          ? `${color} border-opacity-60`
          : 'bg-[#1a1a1a] text-gray-600 border-[hsla(0,0%,15%,1)] hover:bg-[#252525]'
        }`}
    >
      {bit}
    </button>
    ...
    <div className='h-full flex flex-col'>
      ...
      <div className='flex-1 overflow-auto p-4 sm:p-5 md:p-6'>
        <div className='max-w-2xl mx-auto space-y-4'>
          ...
          <Card>
            <CardContent className='pt-6 space-y-5'>
              ...
              <PermRow
                label='Owner'
                perm={perms.owner}
                onChange={(p) => setPerms({ ...perms, owner: p })}
                special={perms.setuid}
                specialLabel='SetUID'
                specialValue={perms.setuid}
                onSpecialChange={(v) => setPerms({ ...perms, setuid: v })}
              />
              <PermRow
                label='Group'
                perm={perms.group}
                onChange={(p) => setPerms({ ...perms, group: p })}
                special={perms.setgid}
                specialLabel='SetGID'
                specialValue={perms.setgid}
                onSpecialChange={(v) => setPerms({ ...perms, setgid: v })}
              />
              <PermRow
                label='Other'
                perm={perms.other}
                onChange={(p) => setPerms({ ...perms, other: p })}
                special={perms.sticky}
                specialLabel='Sticky'
                specialValue={perms.sticky}
                onSpecialChange={(v) => setPerms({ ...perms, sticky: v })}
              />
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className='pt-6 space-y-3'>
              <div className='flex items-center gap-3'>
                <p className='text-xs text-gray-500 uppercase tracking-wide'>chmod Commands</p>
                <Input
                  value={filename}
              ...
              {[
                { label: 'Octal', value: chmodCmd },
                { label: 'Symbolic', value: chmodSymCmd },
              ].map(({ label, value }) => (
                <div key={label} className='flex items-center gap-2'>
                  <span className='text-xs text-gray-500 w-16 shrink-0'>{label}</span>
                  <code className='flex-1 bg-[#121212] rounded px-3 py-1.5 font-mono text-sm text-green-400'>{value}</code>
          ...
          <Card>
            <CardContent className='pt-6 space-y-3'>
              <p className='text-xs text-gray-500 uppercase tracking-wide'>Import from Octal</p>
              <div className='flex gap-2'>
                <Input
                  ...
                  maxLength={4}
                />
                <Button variant='outline' onClick={() => handleOctalInput(octal)}>
                  Sync current
                </Button>
              </div>
          ...
          <Card>
            <CardContent className='pt-6 space-y-3'>
              <p className='text-xs text-gray-500 uppercase tracking-wide'>Common Presets</p>
          ...
          <Card>
            <CardContent className='pt-6 space-y-2'>
              <p className='text-xs text-gray-500 uppercase tracking-wide'>Bit Reference</p>
